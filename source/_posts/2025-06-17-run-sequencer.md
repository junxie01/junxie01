...
目的：捕获局部和全局趋势。
 ...
 3. MST构建与η计算
   对每个（度量, 尺度, 片段）生成距离矩阵 → 构建MST → 计算伸长率$ η_{klm} $。例（文章图3）：窄脉冲数据在小尺度（l=4,5）的欧氏距离获得高η，指示局部趋势。
 4. 信息聚合
   * 片段聚合：对固定（度量,尺度），用η加权平均各片段距离矩阵：$ D_{kl}=⟨η_{klm}⋅D_{klm}⟩_m $
   * 全局聚合：用η加权合并所有MST的边，构建全局邻近矩阵：
$$ P_{combined}=⟨η_{kl}⋅#edges(MST_{kl})⟩_kl $$
 5. 序列提取
   构建全局MST → 从最不连通节点开始广度优先搜索（BFS），生成最终序列顺序。
...
# 使用注意事项
    ...
    * 大数据集（$ N>104 $）：
     ** 启用子采样模式（见7.3节）：
     1. 在小样本（Ns≪N）构建初始序列。
     2. 按锚点（f AN s）逐步插入剩余点。
     ** 复杂度：从O(N2)降至O(NlogN)。
   * 并行化：各（度量,尺度,片段）计算相互独立，可并行加速。
   3. 结果验证
   * 检查高η度量：分析主导趋势的度量（如EMD指示位移，KL散度捕获分布变化）。
   * 残差分析（见5.1节）：
   ** 沿序列方向平滑数据 → 比较原始值与平滑值的残差。
大残差可能指示异常点或次级趋势。
   ** 对比降维方法：用归一化伸长率$ η′=η/N $评估t-SNE/UMAP结果（图7,8）。
   4. 局限性
